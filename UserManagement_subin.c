/*
	작성자: 141033 박수빈
	작성일: 2014.06.01
	설명: 최대한 심플하고 사용하기 편하게 디자인 하려 노력했습니다.

	 1. 실질적인 입력이 필요한 부분을 제외하고는 엔터키를 누를 필요가 없도록 하였으며,
		페이지 넘김은 방향키로, 나가기는 ESC키, 확인은 엔터키를 사용할 수 있게 하였습니다.
...
int main(void)
{
	...
	FILE *readFile;
	FILE *writeFile;
	int input, maxsize, count = -1;		//첫 문장은 제외해야 하기 때문에 -1임
	...
	while (1){
		...
		case 5:	//검색
			...
			break;
		case 6:	//저장
			writeFile = 0;	//값을 넘기기 위해 초기화
			saveInfo(userInfo, writeFile, count);
			break;
		case 7:		//종료
		case -21:	//ESC키로 종료
			puts("\n프로그램을 종료합니다.");	//아직 저장하고 종료하는 기능은 구현하지 않았음
			free(userInfo);
			return 0;
		default:
			puts("\n잘못된 입력입니다.");	//오류 메시지가 남아있지 않고 사라지는 것은 의도한 연출임
			break;
		}
	}
	free(userInfo);
...
void printList(UserInfo userInfo[], int count)
{
	...
	while (1){
		...
		puts("-------------------------------------------------------------------------------");
		puts("\t 페이지 이동 :   <-    ->     \t\t 나가기:  ESC키");
		puts("-------------------------------------------------------------------------------");
		input = getch();
		if (input == 224)	//방향키는 입력 시 224값이 들어온 후 75,77 등의 다른 값이 들어옴
			input = getch();	//그래서 224가 들어올 경우 다시 한번 값을 받도록 하였음
		switch (input)
		{
		case 75:	//좌측 방향키
			if (k != 1) k--;
	...
	while (1){
		...
		if (userInfo[count].userName[1] == 0){
			...
			if (strcmp(userInfo[count].userName, "esc") == 0) return --count;	//입력값이 esc이면 저장하지 않고 바로 나감
				...
				for (int i = 0; i < 20; i++) userInfo[count].userName[i] = 0;	//초기화
				puts("\n\n\t\t 이름이 너무 길거나 짧습니다.");
				puts("\t\t 이름은 2 ~ 3 글자만 가능합니다. (영문 4 ~ 6 글자)");
				while (getchar() != '\n');	//버퍼 제거 및 오류 메세지에서 정지
				continue;
			}
		...
		if (userInfo[count].handphone[1] == 0){
			...
			if (strcmp(userInfo[count].handphone, "esc") == 0) return --count;
				...
				puts("\n\n\t\t 연락처가 너무 길거나 짧습니다. ");
				puts("\t\t 형식에 맞게 입력해주세요. (예: 010-1234-5678)");
				while (getchar() != '\n');
				continue;
			}
